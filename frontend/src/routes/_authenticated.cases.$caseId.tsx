    ...
    <main className="mx-auto max-w-7xl px-6 py-8">
      ...
      <div className="mt-10 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-8">
        ...
        <aside className="space-y-4">
          ...
          <UploadBox
            onUpload={(file, kind) => uploadM.mutate({ file, kind })}
            busy={uploadM.isPending}
          />
              ...
              <ol className="rounded-lg bg-surface ring-1 ring-border/40 divide-y divide-border/40">
                {data.events.map((ev: any) => (
                  <li key={ev.id} className="px-4 py-2.5 flex items-baseline gap-4 text-[12px]">
                    ...
                    <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-primary w-32 shrink-0">
                      {ev.kind.replace(/_/g, " ")}
                    </span>
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-1">
            {c.needs_human_review && (
              <span className="inline-flex items-center gap-1 rounded-full bg-warning/10 text-warning ring-1 ring-warning/30 px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.16em]">
                Review
              </span>
            )}
          </div>
...
function UploadBox({
  ...
  onUpload: (f: File, kind: string) => void;
  busy: boolean;
}) {
  ...
  return (
    <label className="block rounded-md bg-surface-2 ring-1 ring-dashed ring-border/70 p-4 text-center cursor-pointer hover:ring-primary/40 transition-colors">
      ...
      <div className="text-[10px] text-muted-foreground">
        Click to upload · text files extract richest
      </div>
    </label>
  );
}
    ...
    <div className="rounded-xl bg-surface ring-1 ring-border/50 p-10 text-center">
      ...
      <h2 className="mt-3 text-xl font-medium text-foreground">
        {evidenceCount === 0
          ...
          : canCorrelate
            ? "Evidence ready. Run correlation."
            : "Extracting evidence…"}
      </h2>
      {canCorrelate && (
        <button
...
function EvidenceGraph({ evidence, conclusions, links }: any) {
  ...
  const primaryLinks = new Set(
    (links as any[]).filter((l) => l.conclusion_id === primary?.id).map((l) => l.evidence_id),
  );
      ...
      <ol className="relative pl-6">
        ...
        <AnimatePresence>
          {events.slice(0, step + 1).map((ev: any, i: number) => (
            ...
            >
              <span
                className={`absolute -left-[18px] top-3 size-2 rounded-full ${i === step ? "bg-primary shadow-[0_0_10px_oklch(0.62_0.18_258_/_0.7)]" : "bg-foreground/40"}`}
              />
